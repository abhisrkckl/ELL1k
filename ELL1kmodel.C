...
double ELL1kmodel(pulsar *psr,int p,int ipos,int param)
{
    double an,x0,m2,tt0,orbits,phase,e1,e2,dre,drep,drepp,brace,dlogbr,ds,da,pb;
    double omdot,pa,pbdotcalc;
    ...
    const char *CVS_verNum = "$Id: eba3aebd023d9409a1e1406fe0471685d0519a4a $";
    if (displayCVSversion == 1) CVSdisplayVersion("ELL1Kmodel.C","ELL1Kmodel()",CVS_verNum);
    ...
    if (psr[p].param[param_omdot].paramSet[0] == 1) omdot = psr[p].param[param_omdot].val[0]*M_PI/(180*SECDAY*365.25); /* Om_dot */
    else omdot=0.0;
  
    double nb = 2.0*M_PI/pb;
    an = 2.0*M_PI/pb - omdot;
    pa = 2.0*M_PI/an;  // P_a instead of P_b
    ...
    eps1dot =  eps2*omdot;				/* Modification in eps1_dot */	
    eps2dot = -eps1*omdot;  				/* Modification in eps2_dot */
    //These are not used.
    ...
    //e1 = eps1 + eps1dot*tt0;		// Secular variations in Laplace-Lagrange parameters
    //e2 = eps2 + eps2dot*tt0;
    e1 = eps1*cos(omdot*tt0) + eps2*sin(omdot*tt0);
    e2 = eps2*cos(omdot*tt0) - eps1*sin(omdot*tt0);

    dre  = x*(sin(phase)-0.5*(e1*(cos(2.0*phase)+3)-e2*sin(2.0*phase)));	// Roemer delay
    drep = x*cos(phase);
    drepp=-x*sin(phase);

    brace=1-si*sin(phase);
    dlogbr=log(brace);
    ds=-2*m2*dlogbr;			// Shapiro delay

    /* NOTE: a0 and b0 are always zero -- they are not set in the original TEMPO!!!!! */
    da=a0*sin(phase)+b0*cos(phase);  	// Abberation delay -- Not considered
    ...
    Csigma   = x*cos(phase);
    Cx       = sin(phase);
    Ceps1    = -0.5*x*((cos(2*phase)+3)*cos(omdot*tt0) + sin(2*phase)*sin(omdot*tt0));
    Ceps2    = -0.5*x*((cos(2*phase)+3)*sin(omdot*tt0) - sin(2*phase)*cos(omdot*tt0));
    Cm2      = -2*dlogbr;
    Csi      = 2*m2*sin(phase)/brace; 
    Comdot   = -0.5*x*((cos(2*phase)+3)*e2 + sin(2*phase)*e1)*tt0;
